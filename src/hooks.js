...
import { Context, formatForError } from './context.js';
...
export const useState = (initialState, storeKey) => {
  ...
  if (!token) {
    if (storeKey !== undefined && !((typeof storeKey === 'string' && storeKey) || typeof storeKey === 'symbol')) {
      throw new TypeError(
        `Invalid useState key for store "${store.tag}": "${formatForError(storeKey)}", ` +
    ...
    token = { storeKey, setter: store.getSingleKeySetter(storeKey) };
